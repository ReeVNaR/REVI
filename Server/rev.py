...
def get_session(session_id):
    ...
    if not session.get('messages'):
        ...
        return jsonify({'error': 'Session deleted - empty'}), 404
...
def get_relevant_context(user_id, current_message, limit=5):
    """Get relevant messages from all user sessions"""
    all_sessions = db.sessions.find({'user_id': ObjectId(user_id)})
    all_messages = []
    ...
    # Sort by relevance (for now, just getting most recent)
...
def session_chat(session_id):
    ...
    context_message = {
        "role": "model",
        "parts": [{
            "text": "Previous relevant conversations:\n" + 
                   "\n".join([f"{msg['role']}: {msg['parts'][0]['text']}" 
                            for msg in relevant_context])
        }]
    }
    
    # Prepare messages for AI with context
    current_messages = [context_message] if relevant_context else []
    current_messages.extend(session["messages"][-4:])  # Add recent conversation
    ...
    session["messages"].append(current_messages[-1])
    ...
    try:
        ...
        # Update user's memory and session
...
if __name__ == "__main__":
    ...
    # In production, host should bind to 0.0.0.0
